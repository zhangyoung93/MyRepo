...
public class Sort {
     ...
     *
     * 算法推导：
     * 假设待排序数组arr={1,2,3,4}，数组长度n=4；
     * 第1趟排序，将数字"1"浮起("1"分别与"2"、"3"、"4"比较，共3次)，排序结果为{2,3,4,1}。
     * 第2趟排序，将数字"2"浮起("2"分别与"3"、"4"，共2次，无需与"1"比较)，排序结果为{3,4,2,1}。
     * 第3趟排序，将数字"3"浮起("3"分别与"4"比较，共1次，无需与"1"、"2"比较)，排序结果为{4,3,2,1}。
     * 第i趟排序，将第1个数字浮起(arr[0]分别与arr[1]、arr[2]、...、arr[n-i]等比较，共n-i次)
     * 双层嵌套for循环
     * 第一层for循环表示趟数i，i=1到i=n-1。
     * 第二层for循环表示每趟将第1个数字浮起的步骤j，j=0到j=n-i-1。
     *
     * 性能：
     * 时间复杂度：O(n²) --双层嵌套循环
     * 空间复杂度：O(1) --消耗有限的内存资源
     ...
     *
     * 算法推导：
     * 假设待排序数组arr={4,3,2,1}，数组长度n=4，假设arr[0]=4是已排序的数据。
     * 第1次排序，比较arr[1]与arr[0]，共1次，排序结果为{3,4,2,1}
     * 第2次排序，比较arr[2]与arr[0]、arr[2]与arr[1]，共2次，排序结果为{2,3,4,1}
     * 第3次排序，比较arr[3]与arr[0]、arr[3]与arr[1]、arr[3]与arr[2]，共3次，排序结果为{1,2,3,4}
     * 第i次排序，比较arr[i]与arr[0]、...、arr[i]与arr[i-1]，共i次
     * 双层嵌套for循环
     * 第一层for循环遍历未排序的数据，i=1到i=n-1
     * 第二层for循环将未排序的数据逐个与已排序的数据比较，j=0到j=i-1
     *
     * 性能：
     * 时间复杂度：O(n²) --双层嵌套循环
     * 空间复杂度：O(1) --消耗有限的内存资源
     ...
     *
     * 算法推导：
     * 1、一共循环i=n-1次，最后1次无需再判断。
     * 2、每次从循环中查找最值索引，再将最值元素与数组首元素交换。
     * 3、循环n-1次结束。
     *
     * 性能：
     * 时间复杂度：O(n²) --双层嵌套循环
     ...
     *
     * 算法推导：
     * 1、对原数组arr二分，结果为arrLow、arrHigh。
     * 2、对arrLow、arrHigh继续二分。
     * 3、循环步骤2，直到二分结果只有1个元素。
     * 4、对二分结果排序且合并。
     * 5、循环执行直到对原数组完成合并。
     *
     * 性能：
